...
<Callout title="Tuning Tips" type="info">
  -  PedroPathing now no longer activates heading and translational PIDF correction for drive tuning. If you would like to test all three of them, navigate yourself to the `Line Tuner` in the `Manual` folder. Use it to adjust `BreakingStrength`, path constraints and making sure all PIDFs are working well together.
